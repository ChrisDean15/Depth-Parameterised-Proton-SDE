...
int main(int argc, char **argv) {
  ...
  double dt_max, dt_min, dt_adaptive, absorption_e;
  int nrep;
  cfg.lookupValue("maximum_step_size", dt_max);
  cfg.lookupValue("minimum_step_size", dt_min);
  cfg.lookupValue("adaptive_step_threshold", dt_adaptive);
  ...
  proton_path p(initial_e_mean + 3 * initial_e_sd, ((dt_max + dt_min) / 2),
                absorption_e, change_points, interval_materials, materials);
  int n = p.energy.size();
  Grid grid(n * grid_dx / ((dt_max + dt_min) / 2), grid_dx);
  ...
  for (int i = 0; i < nrep; i++) {
    ...
    p.reset(e0, x, w);
    len = p.simulate(dt_max, dt_min, dt_adaptive, absorption_e, change_points,
