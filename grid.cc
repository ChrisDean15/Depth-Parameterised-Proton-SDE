...
struct Grid {
  ...
  template <typename T>
  bool isValidIndex3D(const std::vector<std::vector<std::vector<T>>> &vec,
                      int i, int j, int k) {
    return (i >= 0 && i < static_cast<int>(vec.size())) &&
           (j >= 0 && j < static_cast<int>(vec[i].size())) &&
           (k >= 0 && k < static_cast<int>(vec[i][j].size()));
  }
    ...
    for (unsigned int ix = 0; ix < x_pp.size(); ix++) {
      for (unsigned int j = 0; j < x_pp[ix].size(); j++) {
        for (unsigned int k = 0; k < x_pp[ix][j].size(); k++) {
          if (ix < 2 && isValidIndex3D(x_pp, 2, j, k)) {
            x_pp[ix][j][k] = x_pp[2][j][k];
          }
          if (x_pp[ix][j][k] > 0) {
      ...
      for (unsigned int j = 0; j < x_pm[ix].size(); j++) {
        for (unsigned int k = 0; k < x_pm[ix][j].size(); k++) {
          if (ix < 2 && isValidIndex3D(x_pm, 2, j, k)) {
            x_pm[ix][j][k] = x_pm[2][j][k];
          }
          if (x_pm[ix][j][k] > 0) {
      ...
      for (unsigned int j = 0; j < x_mp[ix].size(); j++) {
        for (unsigned int k = 0; k < x_mp[ix][j].size(); k++) {
          if (ix < 2 && isValidIndex3D(x_mp, 2, j, k)) {
            x_mp[ix][j][k] = x_mp[2][j][k];
          }
          if (x_mp[ix][j][k] > 0) {
      ...
      for (unsigned int j = 0; j < x_mm[ix].size(); j++) {
        for (unsigned int k = 0; k < x_mm[ix][j].size(); k++) {
          if (ix < 2 && isValidIndex3D(x_mm, 2, j, k)) {
            x_mm[ix][j][k] = x_mm[2][j][k];
          }
          if (x_mm[ix][j][k] > 0) {
  ...
  void add(const std::vector<std::vector<double>> &y,
    ...
    std::vector<std::vector<double>> tmp2(1, tmp);
    for (int i = 0; i < len; i++) {
      ix = floor((y[i][0] + y[i + 1][0]) / (2 * dx));
      lin_interp = ((y[i][0] + y[i + 1][0]) / 2 - ix * dx) / dx;
      ...
      if (ix >= 0) {
        ...
        if (y[i][1] >= 0 && y[i][2] >= 0) {
          ...
          x_pp[ix][iy][iz] += (1 - lin_interp) * s[i];
          ...
          x_pm[ix][iy][iz] += (1 - lin_interp) * s[i];
        } else if (y[i][1] < 0 && y[i][2] >= 0) {
          if (iy >= x_mp[ix].size()) {
          ...
          x_mp[ix][iy][iz] += (1 - lin_interp) * s[i];
        } else {
          if (iy >= x_mm[ix].size()) {
          ...
          x_mm[ix][iy][iz] += (1 - lin_interp) * s[i];
        }
      }
      ix++;
      if (ix >= 0) {
