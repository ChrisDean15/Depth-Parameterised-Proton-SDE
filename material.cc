...
struct Atom {
  ...
  void sample_nonelastic_collision(double &e, double &alpha,
    ...
    alpha = out_angle_lab;
    if (out_energy_cm == 0) {
      alpha = 1 - 1e-10; // If outgoing energy is 0, then out_angle_lab should
                         // be 1, rounding errors allow it to be slightly above
                         // 1 which is invalid.
    }
  }
...
struct Material {
  ...
  double bethe_bloch_deriv(const double e) const {
    double mecsq = 0.511;   // mass of electron * speed of light squared, MeV
    double mpcsq = 938.346; // mass of proton * speed of light squared, MeV
    double betasq = (2 * mpcsq + e) * e / pow(mpcsq + e, 2);
    double ret = 0;
    for (unsigned int i = 0; i < at.size(); i++) {
      ret += (x[i] * 0.6144 * at[i].z * density /
              (pow((2 * mpcsq + e) * e, 0.5) * pow(betasq, 0.5) * at[i].a)) *
             ((1 / betasq) - pow(mpcsq, 2) *
                                 log(2 * mecsq * betasq / (I * (1 - betasq))) /
                                 ((2 * mpcsq + e) * e)); // MeV / cm
    }
    return -ret;
  }

  double multiple_scattering_sd(const double e, const double ds) const {
    double dt = 0.05;
    double mpcsq = 938.346; // mass of proton * speed of light squared, MeV
