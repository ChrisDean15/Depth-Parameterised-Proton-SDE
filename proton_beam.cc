...
struct proton_path {
  ...
  double spherical_bm(const double dt, const double time_rescaling, int &ix,
                      gsl_rng *gen, const Material &mat,
                      const double next_change) {
    double time_step = dt;
    if ((next_change - x[ix - 1][0]) < dt) {
      time_step = (next_change - x[ix - 1][0]);
    }
    double underlying_time_step = time_step / time_rescaling;
    z[0] = sin(omega[ix - 1][0]) * cos(omega[ix - 1][1]);
    z[1] = sin(omega[ix - 1][0]) * sin(omega[ix - 1][1]);
    z[2] = cos(omega[ix - 1][0]);
    double y = wright_fisher(
        pow(mat.multiple_scattering_sd(energy[ix - 1], underlying_time_step),
            2),
        gen);
    ...
    if (sin(omega[ix][0]) * cos(omega[ix][1]) < 0.01) {
      omega[ix] = omega[ix - 1];
    }
    double v0 = omega[ix - 1][0];
    double w0 = omega[ix - 1][1];
    x[ix][0] = x[ix - 1][0] + time_step;
    x[ix][1] = x[ix - 1][1] + underlying_time_step * sin(v0) * sin(w0);
    x[ix][2] = x[ix - 1][2] + underlying_time_step * cos(v0);

    double bethe_bloch_update =
        mat.bethe_bloch(energy[ix - 1]) * underlying_time_step;
    energy[ix] = energy[ix - 1] -
                 fmin(fmax(bethe_bloch_update +
                               sqrt(underlying_time_step) *
                                   mat.energy_straggling_sd(energy[ix - 1]) *
                                   gsl_ran_gaussian_ziggurat(gen, 1),
                           0),
                      2 * bethe_bloch_update);
  ...
  int simulate(const double dt_max, const double dt_min,
               const double dt_adaptive, const double absorption_energy,
               const std::vector<double> &change_points,
               const std::vector<int> &interval_materials,
    ...
    while (energy[ix - 1] > absorption_energy) {
      double time_rescaling = sin(omega[ix - 1][0]) * cos(omega[ix - 1][1]);
      time_step = fmax(
          dt_min,
          fmin(dt_max, dt_adaptive * time_rescaling /
                           materials[interval_materials[material_index - 1]]
                               .bethe_bloch_deriv(energy[ix - 1])));
      time_step =
          spherical_bm(time_step, time_rescaling, ix, gen,
                       materials[interval_materials[material_index - 1]],
                       change_points[material_index]);
      if (energy[ix - 1] > absorption_energy) {
        ...
        if (gsl_rng_uniform(gen) <
            1 - exp(-alpha * time_step / time_rescaling)) {
      ...
      if (sin(omega[ix - 1][0]) * cos(omega[ix - 1][1]) < 0.01) {
        omega[ix - 1] = omega[ix - 2];
      }
