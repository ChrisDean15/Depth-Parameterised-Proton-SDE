# Depth-Parameterised-Proton-beam-SDE
This repository houses an adaptation of the simulator found [here](https://github.com/JereKoskela/proton-beam-sde). The cited code simulates an SDE to describe the path of a proton in proton beam therapy parameterised by the proton's track length. This adaptatation applies a time change to describe the path of the proton parameterised by the proton's depth, where the proton's depth is given by v0·x with v0 the initial direction of the proton and x its current position. It is under active development and **not fit for standalone use**.

The following information is taken from the [track length parameterisation](https://github.com/JereKoskela/proton-beam-sde), but we also state it here for ease of use.
